SELECT a.is_admin AS isadm 
       ...
       ,a.role_id 
       ,NVL(a.role_name,' ')        AS role_name 
       ,NVL(a.role_description,' ') AS role_description 
       ,NVL(b.role_name,' ')        AS parent_role 
--       ,NVL(GROUP_CONCAT(c.module_name),' ')       AS modules 
       ,NVL(LISTAGG(c.module_name, ',') within group (order by module_name),' ')       AS modules 
