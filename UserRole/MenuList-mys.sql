SELECT
  ...
  a.MODULE_ID,
  a.MODULE_NAME,
  IFNULL(a.PAGE_NAME, ' ') AS PAGE_NAME,
  IFNULL(b.MODULE_NAME, ' ') AS PARENT_MENU,
  (
    CASE
      a.MENU_LEVEL
      WHEN 1 THEN 'Level 1'
  ...
  a.SORT_ORDER,
  IFNULL(a.FOLDER_PATH, ' ') AS FOLDER_PATH,
  IFNULL(ra.ROLES_UNDER, ' ') as ROLES_UNDER,
  IFNULL(wa.module_id, 0) AS SPCL_AUTH,
  IFNULL(fa.module_id, 0) AS FLDS_AUTH
FROM  APPS_MODULE_REF a
  ...
  LEFT JOIN (
	  SELECT e.MODULE_ID, GROUP_CONCAT(distinct d.ROLE_NAME) as ROLES_UNDER
...
GROUP BY
  ...
  a.FOLDER_PATH,
  a.FOLDER_PATH,
  IFNULL(wa.module_id, 0),
  IFNULL(fa.module_id, 0)
ORDER BY
  a.MENU_LEVEL,
  a.SORT_ORDER,
  a.MODULE_NAME
;
  ...
  IFNULL(
      SELECT GROUP_CONCAT(distinct d.ROLE_NAME)
        FROM APPS_ROLE_REF d
       INNER JOIN APPS_ROLE_MODULE_REF e 
          ON d.ROLE_ID = e.ROLE_ID
         AND e.IS_ACTIVE = 'Y'
        WHERE e.MODULE_ID = a.MODULE_ID
          AND d.IS_ACTIVE = 'Y'
        GROUP BY e.MODULE_ID
     )
     ,' ') 
