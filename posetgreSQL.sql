...
/* ALTER SEQUENCE apps_role_ref_role_id_seq RESTART WITH 0; */
...
ALTER SEQUENCE apps_module_ref_module_id_seq RESTART WITH 100;

/*--------------------------------------------------------
--  function get_userauth_all
--------------------------------------------------------*/
create or replace procedure get_userauth_all(
   pi_uuid int
)
language plpgsql    
as $$
...
--------------------------------------------------------*/
create or replace procedure get_user_module_auth(
   pv_uuid int, pv_pagename varchar(50)
)
language plpgsql    
as $$
begin
